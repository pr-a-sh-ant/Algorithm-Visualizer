...
Algorithm::Algorithm(Array& arr) : array(arr), i(0), j(0), pivotIndex(-1), tempArray(arr.getData().size()) {}
...
void Algorithm::mergeSortRecursive(size_t left, size_t right) {
    if (left < right) {
...
void Algorithm::merge(size_t left, size_t middle, size_t right) {
...
void Algorithm::quickSortRecursive(int low, int high) {
    if (low < high) {
...
int Algorithm::partition(int low, int high) {
...
const std::vector<int>& Algorithm::getCurrentState() const {
    return array.getData();
}

// Implementation of added functions
bool Algorithm::isSortingComplete() const {

    return i >= array.getData().size() - 1;
}

void Algorithm::resetSorting() {
